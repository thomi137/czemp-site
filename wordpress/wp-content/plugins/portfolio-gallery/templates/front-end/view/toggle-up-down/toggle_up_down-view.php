	...
	<div id="huge_it_portfolio_options_and_filters_<?php echo $portfolioID; ?>">
		...
		<?php if ( $portfolioShowSorting == "on" ) { ?>
			<div id="huge_it_portfolio_options_<?php echo $portfolioID; ?>" 
				...
				<ul id="sort-by" class="option-set clearfix" data-option-key="sortBy">
					...
					<li><a href="#sortBy=id"
					       data-option-value="id"><?php echo $paramssld["ht_view0_sorting_name_by_id"]; ?></a>
					</li>
                                        <?php endif;?>
                                        <?php if($paramssld["ht_view0_sorting_name_by_name"] != ''):?>
					<li><a href="#sortBy=symbol"
					       data-option-value="symbol"><?php echo $paramssld["ht_view0_sorting_name_by_name"]; ?></a>
					</li>
                                        <?php endif;?>
                                        <?php if($paramssld["ht_view0_sorting_name_by_random"] != ''):?>
					<li id="shuffle"><a
							href='#shuffle'><?php echo $paramssld["ht_view0_sorting_name_by_random"]; ?></a>
					</li>
                                        <?php endif;?>
				</ul>
				<ul id="port-sort-direction" class="option-set clearfix" data-option-key="sortAscending">
                                        <?php if($paramssld["ht_view0_sorting_name_by_asc"] != ''):?>
					<li><a href="#sortAscending=true" data-option-value="true"
					       class="selected"><?php echo $paramssld["ht_view0_sorting_name_by_asc"]; ?></a>
					</li>
                                        <?php endif;?>
                                        <?php if($paramssld["ht_view0_sorting_name_by_desc"] != ''):?>
					<li><a href="#sortAscending=false"
					       data-option-value="false"><?php echo $paramssld["ht_view0_sorting_name_by_desc"]; ?></a>
					</li>
                                        <?php endif;?>
				</ul>
			</div>
		...
		if ( $portfolioShowFiltering == "on" ) { ?>
			<div id="huge_it_portfolio_filters_<?php echo $portfolioID; ?>"
			     data-filtering-position="<?php echo $paramssld["ht_view0_filtering_float"]; ?>">
				<ul>
...
<?php } ?>
	<div id="huge_it_portfolio_container_<?php echo $portfolioID; ?>"
	     data-show-loading="<?php echo $portfolioShowLoading; ?>" data-show-center="<?php echo $portfolioposition; ?>"
	     class="huge_it_portfolio_container super-list variable-sizes clearfix view-<?php echo $view_slug;?>" <?php if ( $paramssld["ht_view0_sorting_float"] == "top" && $paramssld["ht_view0_filtering_float"] == "top" ) {
		...
		foreach ( $images as $key => $row ) {
			...
			<div
				...
				<div class="wd-portfolio-panel wd-portfolio-panel_<?php echo $portfolioID; ?>" id="panel<?php echo $key; ?>">
					...
					<?php }
					if ( $paramssld['ht_view0_show_description'] == 'on' && $row->description != '') {
						?>
					...
					if ( $paramssld['ht_view0_show_linkbutton'] == 'on' && $link != '' ) {
						...
						<div class="button-block">
							<a href="<?php echo esc_url( $link ); ?>" <?php if ( $row->link_target == "on" ) {
								echo 'target="_blank"';
							} ?>><?php echo $paramssld['ht_view0_linkbutton_text']; ?></a>
						</div>
					<?php } ?>
				</div>
			</div>
