		...
		<?php if ( $portfolioShowSorting == "on" ) { ?>
			<div id="huge_it_portfolio_options_<?php echo $portfolioID; ?>"
				...
				<ul id="port-sort-direction" class="option-set clearfix" data-option-key="sortAscending">
					<?php if ( $paramssld["ht_view1_sorting_name_by_asc"] != '' ): ?>
...
<?php } ?>
	<div id="huge_it_portfolio_container_<?php echo $portfolioID; ?>"
	     ...
	     data-show-center="<?php echo $portfolioposition; ?>" <?php if ( $paramssld["ht_view1_sorting_float"] == "top" && $paramssld["ht_view1_filtering_float"] == "top" ) {
									   ...
									   title="<?php echo esc_attr( $row->name ); ?>">
										<img alt="<?php echo esc_attr( $row->name ); ?>"
										     ...
										     src="<?php echo esc_url( portfolio_gallery_get_image_by_sizes_and_src( $imgurl[0], array(
											     $paramssld['ht_view1_block_width'],
											     ''
										     ), false ) ); ?>"/>
									</a>
									<?php
											...
											case 'image':
												...
												<a href="<?php echo esc_url( $img ); ?>"
												   ...
												   title="<?php esc_attr( huge_it_title_img_display( $img, $title ) ); ?>"><img
														src="<?php echo esc_url( portfolio_gallery_get_image_by_sizes_and_src( $img, $paramssld['ht_view1_thumbs_width'], true ) ); ?>"></a>
												<?php
												break;
											case 'youtube':
					...
					if ( $paramssld['ht_view1_show_linkbutton'] == 'on' && $link != '' ) {
						...
						<div class="button-block">
							<a href="<?php echo esc_url( $link ); ?>" <?php if ( $row->link_target == "on" ) {
								echo 'target="_blank"';
							} ?>><?php echo $paramssld['ht_view1_linkbutton_text']; ?></a>
						</div>
					<?php } ?>
				</div>
			</div>
